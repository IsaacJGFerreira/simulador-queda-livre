...
st.set_page_config(page_title="Simulador de Queda Livre", page_icon="🪂", layout="wide")
...
st.markdown(
    ...
    .hero-card {
        ...
        padding: 26px 32px; border-radius: 22px; color: #FFFFFF;
        box-shadow: 0 18px 45px rgba(15, 23, 42, 0.24); margin-bottom: 18px;
        border: 1px solid rgba(255, 255, 255, 0.18);
    }
    .hero-title {font-size: 2.35rem; font-weight: 850; margin-bottom: 8px; letter-spacing: -0.03em; color: #FFFFFF;}
    .hero-subtitle {font-size: 1.08rem; color: rgba(255, 255, 255, 0.94); max-width: 1050px; line-height: 1.45;}
    .section-card {
        background: var(--secondary-background-color); border: 1px solid rgba(148, 163, 184, 0.35);
        border-radius: 18px; padding: 18px 22px; box-shadow: 0 10px 28px rgba(15, 23, 42, 0.10);
        margin: 14px 0 18px 0;
    }
    .section-title {font-size: 1.28rem; font-weight: 780; color: var(--text-color); margin-bottom: 4px;}
    .section-subtitle {font-size: 0.98rem; color: var(--text-color); opacity: 0.82; line-height: 1.45;}
    div[data-testid="stMetric"] {
        background: var(--secondary-background-color); border: 1px solid rgba(148, 163, 184, 0.35);
        border-radius: 16px; padding: 14px 16px; box-shadow: 0 8px 20px rgba(15, 23, 42, 0.08);
    }
    div[data-testid="stMetricLabel"] {color: var(--text-color) !important; opacity: 0.72;}
    div[data-testid="stMetricValue"] {color: var(--text-color) !important; font-weight: 780;}
    [data-testid="stSidebar"] {background: var(--secondary-background-color); border-right: 1px solid rgba(148, 163, 184, 0.25);}
    .tip-box {
        background: rgba(59, 130, 246, 0.12); border: 1px solid rgba(59, 130, 246, 0.28);
        border-left: 5px solid #3B82F6; color: var(--text-color); padding: 14px 18px;
        border-radius: 14px; margin: 12px 0 18px 0; line-height: 1.45;
    }
    </style>
    """,
...
def resolve_plot_theme(theme_choice: str) -> dict[str, str]:
    if theme_choice == "Automático":
        is_dark = current_streamlit_theme_base() != "light"
    else:
        is_dark = theme_choice == "Escuro"

    if is_dark:
        return {
            "template": "plotly_dark", "paper_bg": DARK_BG, "plot_bg": DARK_BG,
            "panel_bg": DARK_PANEL, "grid": DARK_GRID, "text": DARK_TEXT,
            "scene_bg": "#EAF7FF", "scene_text": LIGHT_TEXT, "scene_panel": "#FFFFFF",
        }
    return {
        "template": "plotly_white", "paper_bg": LIGHT_BG, "plot_bg": LIGHT_BG,
        "panel_bg": LIGHT_PANEL, "grid": LIGHT_GRID, "text": LIGHT_TEXT,
        "scene_bg": "#EAF7FF", "scene_text": LIGHT_TEXT, "scene_panel": "#FFFFFF",
    }
...
def format_panel_text(row: pd.Series, object_name: str, gravity: float) -> str:
    return (
        ...
        f"↔ <b>Distância horizontal:</b> {row['distancia_horizontal']:.2f} m<br>"
...
def vector_geometry(row: pd.Series, vector_scale: float, max_speed: float, max_abs_vx: float, max_abs_vy: float) -> dict[str, object]:
    x = float(row["x"])
    y = float(row["y"])
    vx = float(row["vx"])
    vy = float(row["vy"])
...
def build_vector_traces(row: pd.Series, vector_data: dict[str, object]) -> list[go.Scatter]:
    ...
    return [
        go.Scatter(x=[x, v_end[0]], y=[y, v_end[1]], mode="lines", line=dict(width=vector_data["resultant_width"], color=VECTOR_RESULTANT_COLOR), hoverinfo="skip", showlegend=False),
        go.Scatter(x=[v_end[0]], y=[v_end[1]], mode="markers+text", text=["v"], textposition="top center", textfont=dict(size=15, color=VECTOR_RESULTANT_COLOR), marker=dict(size=14, color=VECTOR_RESULTANT_COLOR, symbol="diamond"), hovertemplate=f"|v| = {row['speed']:.2f} m/s<extra></extra>", showlegend=False),
        go.Scatter(x=[x, vx_end[0]], y=[y, vx_end[1]], mode="lines", line=dict(width=vector_data["vx_width"], color=VECTOR_X_COLOR, dash="dot"), hoverinfo="skip", showlegend=False),
        go.Scatter(x=[vx_end[0]], y=[vx_end[1]], mode="markers+text", text=["vₓ"], textposition="top center", textfont=dict(size=15, color=VECTOR_X_COLOR), marker=dict(size=14, color=VECTOR_X_COLOR, symbol=vector_data["vx_symbol"]), hovertemplate=f"vₓ = {row['vx']:.2f} m/s<extra></extra>", showlegend=False),
        go.Scatter(x=[x, vy_end[0]], y=[y, vy_end[1]], mode="lines", line=dict(width=vector_data["vy_width"], color="#DC2626", dash="dot"), hoverinfo="skip", showlegend=False),
        go.Scatter(x=[vy_end[0]], y=[vy_end[1]], mode="markers+text", text=["vᵧ"], textposition="middle right", textfont=dict(size=15, color="#DC2626"), marker=dict(size=14, color="#DC2626", symbol=vector_data["vy_symbol"]), hovertemplate=f"vᵧ = {row['vy']:.2f} m/s<extra></extra>", showlegend=False),
    ]


def style_axes(fig: go.Figure, row: int, col: int, x_title: str, y_title: str, theme: dict[str, str]) -> None:
    fig.update_xaxes(title=x_title, gridcolor=theme["grid"], zerolinecolor=theme["grid"], color=theme["text"], title_font=dict(color=theme["text"]), tickfont=dict(color=theme["text"]), fixedrange=True, row=row, col=col)
    fig.update_yaxes(title=y_title, gridcolor=theme["grid"], zerolinecolor=theme["grid"], color=theme["text"], title_font=dict(color=theme["text"]), tickfont=dict(color=theme["text"]), fixedrange=True, row=row, col=col)
...
def add_local_graph_legends(fig: go.Figure, theme: dict[str, str]) -> None:
    ...
    distance_legend = f"<span style='color:{DISTANCE_COLOR}'><b>■</b></span> distância horizontal percorrida"
    for annotation in [
        ...
        fig.add_annotation(**annotation, showarrow=False, align="left", xanchor="left", yanchor="top", bgcolor="rgba(0,0,0,0)", borderpad=2, font=dict(size=12, color=theme["text"]))


def build_animation_with_graphs(full_df: pd.DataFrame, object_name: str, gravity: float, theme: dict[str, str]) -> go.Figure:
    ...
    distance_min = 0.0
    distance_max = max(float(anim_df["distancia_horizontal"].max()), 1.0)
    distance_margin = max(0.5, 0.08 * distance_max)
    ...
    initial_vector_data = vector_geometry(initial, vector_scale, max_speed, max_abs_vx, max_abs_vy)
    ...
    fig = make_subplots(
        ...
        subplot_titles=("Animação visual da queda livre com vetores", "Painel da queda", "Componentes da velocidade × tempo", "Altura × tempo", "Distância horizontal percorrida × tempo"),
    )
    ...
    fig.add_trace(go.Scatter(x=[initial["x"]], y=[initial["y"]], mode="markers", marker=dict(size=34, color=OBJECT_COLOR, line=dict(width=3, color="#BF360C")), hovertemplate="Tempo: %{customdata[0]:.2f} s<br>Altura: %{y:.2f} m<br>Velocidade: %{customdata[1]:.2f} m/s<extra></extra>", customdata=[[initial["t"], initial["speed"]]], showlegend=False), row=1, col=1)
    for trace in build_vector_traces(initial, initial_vector_data):
        fig.add_trace(trace, row=1, col=1)
    fig.add_trace(go.Scatter(x=[0.07], y=[0.50], mode="text", text=[format_panel_text(initial, object_name, gravity)], textposition="middle left", textfont=dict(size=15, color=LIGHT_TEXT), hoverinfo="skip", showlegend=False), row=1, col=3)
    ...
    fig.add_trace(go.Scatter(x=[initial["t"], initial["t"], initial["t"]], y=[initial["speed"], initial["vx"], initial["vy"]], mode="markers", marker=dict(size=9, color=[VECTOR_RESULTANT_COLOR, VECTOR_X_COLOR, VECTOR_Y_COLOR], line=dict(width=1, color=theme["text"])), hoverinfo="skip", showlegend=False), row=2, col=1)
    ...
    fig.add_trace(go.Scatter(x=[initial["t"]], y=[initial["distancia_horizontal"]], mode="lines", line=dict(width=4, color=DISTANCE_COLOR), showlegend=False), row=2, col=3)
    fig.add_trace(go.Scatter(x=[initial["t"]], y=[initial["distancia_horizontal"]], mode="markers", marker=dict(size=9, color=DISTANCE_COLOR, line=dict(width=1, color=theme["text"])), hoverinfo="skip", showlegend=False), row=2, col=3)
        ...
        frames.append(
            go.Frame(
                ...
                data=[
                    ...
                    go.Scatter(x=[row["x"]], y=[row["y"]], mode="markers", marker=dict(size=34, color=OBJECT_COLOR, line=dict(width=3, color="#BF360C")), hovertemplate="Tempo: %{customdata[0]:.2f} s<br>Altura: %{y:.2f} m<br>Velocidade: %{customdata[1]:.2f} m/s<extra></extra>", customdata=[[row["t"], row["speed"]]], showlegend=False),
                    *build_vector_traces(row, vector_data),
                    ...
                    go.Scatter(x=chart_df["t"], y=chart_df["distancia_horizontal"], mode="lines", line=dict(width=4, color=DISTANCE_COLOR), showlegend=False),
                    go.Scatter(x=[row["t"]], y=[row["distancia_horizontal"]], mode="markers", marker=dict(size=9, color=DISTANCE_COLOR, line=dict(width=1, color=theme["text"])), hoverinfo="skip", showlegend=False),
                ],
            )
        )
        if index % 6 == 0 or index == len(anim_df) - 1:
            slider_steps.append({"method": "animate", "label": f"{row['t']:.1f}s", "args": [[frame_name], {"mode": "immediate", "frame": {"duration": 0, "redraw": True}, "transition": {"duration": 0}}]})
    ...
    fig.update_layout(
        template=theme["template"], height=900, margin=dict(l=20, r=20, t=64, b=60), showlegend=False,
        dragmode=False, plot_bgcolor=theme["plot_bg"], paper_bgcolor=theme["paper_bg"], font=dict(color=theme["text"]),
        updatemenus=[{"type": "buttons", "showactive": False, "direction": "left", "x": 0.02, "y": -0.075, "xanchor": "left", "yanchor": "top", "bgcolor": "#1D4ED8", "bordercolor": "#1E40AF", "borderwidth": 2, "font": {"color": "#FFFFFF", "size": 14}, "buttons": [{"label": "▶ Play", "method": "animate", "args": [None, {"frame": {"duration": ANIMATION_FRAME_DURATION_MS, "redraw": True}, "fromcurrent": True, "transition": {"duration": 0}, "mode": "immediate"}]}, {"label": "⏸ Pausar", "method": "animate", "args": [[None], {"frame": {"duration": 0, "redraw": False}, "transition": {"duration": 0}, "mode": "immediate"}]}]}],
        sliders=[{"active": 0, "x": 0.18, "y": -0.075, "xanchor": "left", "yanchor": "top", "len": 0.76, "pad": {"t": 0, "b": 0}, "bgcolor": "#E5E7EB", "activebgcolor": "#1D4ED8", "bordercolor": "#CBD5E1", "currentvalue": {"prefix": "Tempo: ", "suffix": "", "font": {"size": 14, "color": theme["text"]}}, "steps": slider_steps}],
    )
    ...
    style_axes(fig, row=2, col=3, x_title="Tempo (s)", y_title="Distância horizontal percorrida (m)", theme=theme)
    ...
    fig.update_yaxes(range=[0, distance_max + distance_margin], row=2, col=3)
    return fig


def apply_theme_to_fig(fig: go.Figure, theme: dict[str, str]) -> go.Figure:
    fig.update_layout(template=theme["template"], paper_bgcolor=theme["paper_bg"], plot_bgcolor=theme["plot_bg"], font=dict(color=theme["text"]), legend=dict(font=dict(color=theme["text"]), bgcolor="rgba(0,0,0,0)", orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0))
...
with st.sidebar:
    ...
    theme_choice = st.radio("Tema dos gráficos", options=["Automático", "Escuro", "Claro"], index=0, help="Use Escuro se estiver usando o modo escuro do app e algum gráfico não acompanhar automaticamente.")
    resolved_theme = resolve_plot_theme(theme_choice)

    preset_name = st.selectbox("Objeto visual", options=list(OBJECT_PRESETS.keys()) + ["Personalizado"], index=1 if "Bola de futebol" in OBJECT_PRESETS else 0, help="Na queda livre ideal, a massa e o formato não alteram o tempo de queda. O objeto serve apenas como representação visual.")
    ...
    initial_vx = st.number_input("Velocidade horizontal inicial (m/s)", value=10.0, step=1.0, help="Valor diferente de zero gera distância horizontal linear com o tempo. Use zero para queda vertical pura.")
...
sim_config = SimulationConfig(initial_height=initial_height, initial_vx=initial_vx, initial_vy=initial_vy, gravity=gravity, dt=dt, max_time=max_time, use_air_resistance=False)
...
st.markdown(
    """
    <div class="tip-box">
        <b>Dica didática:</b> a distância horizontal é dada por <b>dₓ = |x - x₀|</b>. Como não há resistência do ar, se <b>vₓ</b> for diferente de zero, esse gráfico é uma reta crescente.
    </div>
    """,
    unsafe_allow_html=True,
)
...
st.markdown(
    ...
    <div class="section-card">
        ...
        <div class="section-subtitle">Estes gráficos mostram o resultado final completo da simulação, sem depender da animação.</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with left:
    ...
    fig_speed = px.line(velocity_df, x="t", y=["|v| velocidade resultante", "vₓ componente horizontal", "vᵧ componente vertical"], labels={"t": "Tempo (s)", "value": "Velocidade (m/s)", "variable": "Grandeza"}, color_discrete_sequence=[VECTOR_RESULTANT_COLOR, VECTOR_X_COLOR, VECTOR_Y_COLOR])
...
with right:
    ...
    fig_distance = px.line(df, x="t", y="distancia_horizontal", labels={"t": "Tempo (s)", "distancia_horizontal": "Distância horizontal percorrida (m)"})
...
with st.expander("Interpretação física"):
    st.markdown(
        ...
        - A distância horizontal percorrida é `dₓ = |x - x₀|`.
        - Como `vₓ` é constante, `dₓ` cresce linearmente com o tempo quando `vₓ ≠ 0`.
