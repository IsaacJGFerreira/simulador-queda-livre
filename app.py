    ...
    <head>
      ...
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        .animation-title-area, .fall-panel-title, .section-title-row { display: flex; align-items: center; gap: 12px; }
        .animation-title-area h2, .fall-panel-title h2 { font-size: 20px; font-weight: 700; margin: 0; color: var(--text-primary); }
        .animation-icon, .panel-icon, .section-icon {
          width: 40px; height: 40px; border-radius: 10px;
          background: rgba(38, 135, 255, 0.14); color: var(--vx-color);
        ...
        .animation-controls button, .speed-control {
          ...
          border: 1px solid #2a4864; color: var(--text-primary);
        ...
        .scene-container {
          ...
          background: var(--scene-bg); border-radius: 12px; overflow: hidden;
        ...
        .vector-legend {
          ...
          display: grid; grid-template-columns: repeat(2, auto); gap: 10px 18px;
          background: rgba(255, 255, 255, 0.55); border: 1px solid rgba(80, 120, 160, 0.18);
        ...
        .data-row, .feature-row {
          ...
          border: 1px solid #263f59; border-radius: 9px; padding: 12px 14px; margin-bottom: 8px;
        }
    ...
    <body>
      ...
      <script>
        ...
        function setGravityVectorStatic() {
          ...
          gLabel.setAttribute('x', 890);
          gLabel.setAttribute('y', 195);
        }
        function drawFrame(index) {
          ...
          if (hasHorizontal) {
            ...
            resultLabel.setAttribute('x', p.x + 34);
            resultLabel.setAttribute('y', p.y + resultYLen + 8);
          }
...
def add_dashboard_background(fig: go.Figure) -> None:
    ...
    fig.add_shape(type="rect", xref="x domain", yref="y domain", x0=0, x1=1, y0=0.10, y1=0.90, fillcolor=BG_CARD, line=dict(color="#1d3346", width=1), layer="below")
    for separator_x in [0.25, 0.50, 0.75]:
        fig.add_shape(type="line", xref="x domain", yref="y domain", x0=separator_x, x1=separator_x, y0=0.25, y1=0.75, line=dict(color="#1d3346", width=1), layer="above")
    for xref, yref in [("x2 domain", "y2 domain"), ("x3 domain", "y3 domain"), ("x4 domain", "y4 domain")]:
        fig.add_shape(type="rect", xref=xref, yref=yref, x0=-0.14, x1=1.14, y0=-0.28, y1=1.34, fillcolor=BG_CARD, line=dict(color=BORDER_CARD, width=1), layer="below")
        fig.add_shape(type="rect", xref=xref, yref=yref, x0=-0.14, x1=1.14, y0=-0.28, y1=1.34, fillcolor="rgba(12, 25, 38, 0.35)", line=dict(width=0), layer="below")
...
def add_summary_annotations(fig: go.Figure, initial_height: float, impact_time: float, initial_vx: float) -> None:
    ...
    for x_icon, x_text, icon, title, value, color, bg in items:
        fig.add_annotation(xref="x domain", yref="y domain", x=x_icon, y=0.50, text=f"<b>{icon}</b>", showarrow=False, font=dict(size=20, color=color), bgcolor=bg, bordercolor=bg, borderpad=8)
        fig.add_annotation(xref="x domain", yref="y domain", x=x_text, y=0.61, text=title, showarrow=False, xanchor="left", align="left", font=dict(size=12, color="#d7e2ee"))
        fig.add_annotation(xref="x domain", yref="y domain", x=x_text, y=0.39, text=f"<b>{value}</b>", showarrow=False, xanchor="left", align="left", font=dict(size=18, color=color))
...
def add_chart_header(fig: go.Figure, xref: str, yref: str, icon: str, title: str, icon_color: str, icon_bg: str, legend_html: str) -> None:
    ...
    fig.add_annotation(xref=xref, yref=yref, x=-0.08, y=1.23, text=f"<b>{icon}</b>", showarrow=False, xanchor="left", yanchor="middle", font=dict(size=15, color=icon_color), bgcolor=icon_bg, bordercolor=icon_bg, borderpad=7)
    fig.add_annotation(xref=xref, yref=yref, x=0.02, y=1.23, text=f"<b>{title}</b>", showarrow=False, xanchor="left", yanchor="middle", align="left", font=dict(size=14, color=TEXT_PRIMARY))
    fig.add_annotation(xref=xref, yref=yref, x=0.02, y=1.10, text=legend_html, showarrow=False, xanchor="left", yanchor="top", align="left", font=dict(size=12, color="#dce7f3"))


def add_chart_headers(fig: go.Figure) -> None:
    """Adiciona cabeçalhos aos três gráficos."""
    velocity_legend = f"<span style='color:{PURPLE}'><b>■</b></span> |v| &nbsp;&nbsp;&nbsp;<span style='color:{BLUE}'><b>■</b></span> Vx &nbsp;&nbsp;&nbsp;<span style='color:{PINK}'><b>■</b></span> Vy"
...
def style_chart_axes(fig: go.Figure, row: int, col: int, x_title: str, y_title: str) -> None:
    ...
    fig.update_xaxes(title=x_title, title_font=dict(size=13, color="#edf4fb"), tickfont=dict(size=12, color="#dbe7f3"), color="#dbe7f3", gridcolor=GRID_COLOR, gridwidth=1, griddash="dash", zerolinecolor=AXIS_COLOR, linecolor=AXIS_COLOR, showline=True, fixedrange=True, row=row, col=col)
    fig.update_yaxes(title=y_title, title_font=dict(size=13, color="#edf4fb"), tickfont=dict(size=12, color="#dbe7f3"), color="#dbe7f3", gridcolor=GRID_COLOR, gridwidth=1, griddash="dash", zerolinecolor=AXIS_COLOR, linecolor=AXIS_COLOR, showline=True, fixedrange=True, row=row, col=col)


def make_value_label_trace(row: pd.Series) -> go.Scatter:
    """Cria etiquetas dos valores atuais no gráfico das velocidades."""
    return go.Scatter(x=[row["t"], row["t"], row["t"]], y=[row["speed"], row["vx"], row["vy"]], mode="text", text=[format_br(float(row["speed"]), 1), format_br(float(row["vx"]), 1), format_br(float(row["vy"]), 1)], textposition=["middle right", "middle right", "middle right"], textfont=dict(color=TEXT_PRIMARY, size=11), hoverinfo="skip", showlegend=False)
...
def build_professional_dashboard(full_df: pd.DataFrame, initial_height: float, initial_vx: float) -> go.Figure:
    ...
    fig = make_subplots(rows=2, cols=3, specs=[[{"colspan": 3}, None, None], [{}, {}, {}]], row_heights=[0.22, 0.78], horizontal_spacing=0.06, vertical_spacing=0.22)
    ...
    fig.add_trace(go.Scatter(x=[initial["t"], initial["t"], initial["t"]], y=[initial["speed"], initial["vx"], initial["vy"]], mode="markers", marker=dict(size=9, color=[PURPLE, BLUE, PINK], line=dict(width=1, color=TEXT_PRIMARY)), hoverinfo="skip", showlegend=False), row=2, col=1)
    ...
    for index, row in anim_df.iterrows():
        ...
        frames.append(go.Frame(name=frame_name, data=[
            ...
            go.Scatter(x=[row["t"], row["t"], row["t"]], y=[row["speed"], row["vx"], row["vy"]], mode="markers", marker=dict(size=9, color=[PURPLE, BLUE, PINK], line=dict(width=1, color=TEXT_PRIMARY)), hoverinfo="skip", showlegend=False),
    ...
    fig.update_layout(template="plotly_dark", height=650, margin=dict(l=28, r=28, t=24, b=72), paper_bgcolor=BG_MAIN, plot_bgcolor=BG_CARD_SECONDARY, font=dict(color=TEXT_PRIMARY, family="Inter, Arial, sans-serif"), showlegend=False, dragmode=False,
        updatemenus=[{"type": "buttons", "showactive": False, "direction": "left", "x": 0.02, "y": -0.09, "xanchor": "left", "yanchor": "top", "bgcolor": "#1D4ED8", "bordercolor": "#1E40AF", "borderwidth": 2, "font": {"color": "#FFFFFF", "size": 14}, "buttons": [
            {"label": "▶ Play", "method": "animate", "args": [None, {"frame": {"duration": ANIMATION_FRAME_DURATION_MS, "redraw": True}, "fromcurrent": True, "transition": {"duration": 0}, "mode": "immediate"}]},
            {"label": "⏸ Pausar", "method": "animate", "args": [[None], {"frame": {"duration": 0, "redraw": False}, "transition": {"duration": 0}, "mode": "immediate"}]},
        ]}],
        sliders=[{"active": 0, "x": 0.18, "y": -0.09, "xanchor": "left", "yanchor": "top", "len": 0.76, "pad": {"t": 0, "b": 0}, "bgcolor": "#1d3346", "activebgcolor": BLUE, "bordercolor": BORDER_CARD, "currentvalue": {"prefix": "Tempo: ", "suffix": "", "font": {"size": 14, "color": TEXT_PRIMARY}}, "steps": slider_steps}],
    )
...
def apply_dark_chart_style(fig: go.Figure) -> go.Figure:
    ...
    fig.update_layout(template="plotly_dark", paper_bgcolor=BG_MAIN, plot_bgcolor=BG_CARD_SECONDARY, font=dict(color=TEXT_PRIMARY), legend=dict(font=dict(color=TEXT_PRIMARY), bgcolor="rgba(0,0,0,0)", orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0))
    fig.update_xaxes(gridcolor=GRID_COLOR, griddash="dash", zerolinecolor=AXIS_COLOR, color=TEXT_PRIMARY, title_font=dict(color=TEXT_PRIMARY), tickfont=dict(color=TEXT_SECONDARY))
    fig.update_yaxes(gridcolor=GRID_COLOR, griddash="dash", zerolinecolor=AXIS_COLOR, color=TEXT_PRIMARY, title_font=dict(color=TEXT_PRIMARY), tickfont=dict(color=TEXT_SECONDARY))
    return fig
...
with st.sidebar:
    ...
    initial_vx = st.number_input("Velocidade horizontal inicial (m/s)", value=0.0, step=1.0, help="Use zero para queda livre vertical. Valores diferentes de zero geram deslocamento horizontal linear.")
...
st.markdown(
    ...
    <div class="section-card">
        ...
        <div class="section-subtitle">
            A animação agora segue a posição horizontal real da simulação. Com Vx = 0, o movimento é vertical; com Vx diferente de zero, aparece deslocamento lateral. O vetor g fica fixo na cena.
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
animation_html = build_animation_panel_html(df=df, object_name=falling_object.name, initial_height=initial_height, initial_vx=initial_vx, gravity=gravity)
...
st.markdown(
    ...
    <div class="section-card">
        ...
        <div class="section-subtitle">
            Os três gráficos abaixo mostram velocidade, altura e distância horizontal, com a possibilidade de acompanhar a construção das curvas pelo botão Play do painel gráfico.
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
panel_fig = build_professional_dashboard(full_df=df, initial_height=initial_height, initial_vx=initial_vx)
st.plotly_chart(panel_fig, use_container_width=True, config=ANIMATION_PLOTLY_CONFIG)

st.markdown(
    """
    <div class="tip-box">
        <b>Observação:</b> se <b>Vx = 0</b>, o corpo cai em linha vertical e a distância horizontal permanece em zero. Se <b>Vx ≠ 0</b>, o corpo se desloca lateralmente e o gráfico da distância horizontal é uma reta crescente.
    </div>
    """,
    unsafe_allow_html=True,
)
...
with left:
    ...
    velocity_df = df[["t", "speed", "vx", "vy"]].rename(columns={"speed": "|v| velocidade resultante", "vx": "Vx componente horizontal", "vy": "Vy componente vertical"})
    fig_speed = px.line(velocity_df, x="t", y=["|v| velocidade resultante", "Vx componente horizontal", "Vy componente vertical"], labels={"t": "Tempo (s)", "value": "Velocidade (m/s)", "variable": "Grandeza"}, color_discrete_sequence=[PURPLE, BLUE, PINK])
...
with right:
    ...
    fig_distance = px.line(df, x="t", y="distancia_horizontal", labels={"t": "Tempo (s)", "distancia_horizontal": "Distância horizontal (m)"})
...
with st.expander("Interpretação física"):
    st.markdown(
        """
        - A animação usa a posição horizontal `x` calculada pela simulação.
        ...
        - O vetor `g` representa a aceleração da gravidade e fica fixo no painel, pois não é um vetor velocidade preso ao corpo.
